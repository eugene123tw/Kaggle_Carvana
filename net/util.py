...
def concatenate_predictions(target_predictions):
    predictions = np.load(target_predictions[0])

    for target in target_predictions[1:]:
        predictions = np.concatenate(predictions,target)
